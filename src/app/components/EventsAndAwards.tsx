      ...
      {!selectedEvent ? (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {events.map((event) => (
              <div key={event.id} className="aur-card p-6 flex flex-col h-full hover:border-[var(--aur-primary)] transition-colors">
                ...
                {showRegistrationForm ? (
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <h3 className="text-xl font-bold text-[var(--aur-text)] mb-6 flex items-center gap-2">
                      <Users className="w-5 h-5 text-[#94061e] dark:text-cyber-yellow" /> Complete Your Registration
                    </h3>
